...
| --- | --- | --- | --- | --- | --- |
| 4736-101 | 129 | `10.179.7.1` | ⚪ UNKNOWN — CCU UNREACHABLE 2026-07-09 (SSH+ICMP dead, VPN verified OK via box1-t47) — train-side power/cellular | ❓ | ⚠️ **in revenue service since KAD 25.06 (KISS calendar, Garnitur 4736-001) with no commissioning record.** Probe attempt 2026-07-09: unreachable. PRIORITY when online — full initial visit / commissioning. Prior: CCU IP confirmed 2026-06-30 (box1-t7) |
...
| 4736-107 | 135 | `10.179.25.1` | ⚪ UNKNOWN — CCU UNREACHABLE 2026-07-09 (SSH+ICMP dead, VPN verified OK) — train-side power/cellular | ❓ | ⚠️ **in revenue service since KAD 07.07 (KISS calendar, Garnitur 4736-007) with no commissioning record.** Probe attempt 2026-07-09: unreachable. PRIORITY when online — full initial visit / commissioning. Prior: CCU IP confirmed 2026-06-30 (box1-t25) |
...
| 4734-106 | 6 | `10.179.31.1` | 🟢 **DONE** — 2026-07-09 commissioned (AR): **12/12 sw v8-006 fw 7.4.2 ✅, 16/16 APs at 6.11.2-0 ✅, OBN 11/11 ✅ persisted (run3/id335)**. Session: bug11 applied on nd-obn 2.2.23 (marker lib/device/vendor/westermo.py); Form-1 `{%- set train_id = 6 -%}` in all 12 nv4-*.cfg (backbone train_id:31 render trap DEFUSED); NDSU .dont rename (auto-update blocked); all persisted via single chroot promote + reboot, re-verified across a second (consist) power-cycle ~08:57Z. AP fw: 6 pushed serial 07:44–08:31Z, 5 activated first pass incl all 3 m-variants; AP4-v1 unit (MAC 00:14:5a:04:62:a7) needed one clean retry after TFTP-helper re-arm (power-cycle had wiped helper + killed retry1) → activated 10:22Z, verify 16/16. vlan7 172.19.131.2/17 ✅, FW Q1 ARP REACHABLE ✅ (Q2/Q3 not run). ⚠️ TFTP CT helper is runtime-only — re-apply before any future AP push. Last touched 2026-07-09 AR | ❓ | KAS 14.07 quality gate: **network commissioning COMPLETE**. Optional: L2 health check + customer report (not pipeline-mandated); Phase-6 Q2/Q3 FW commission-state test if Stadler status wanted. |
...
| 4734-110 | 10 | `10.179.36.1` | ⚪ UNKNOWN — CCU UNREACHABLE 2026-07-09 (SSH+ICMP dead, VPN verified OK via box1-t47) — train-side power/cellular | ❓ | ⚠️ KAS 21.07 (KISS calendar, Garnitur 4734-010) → IoB quality gate ~14.07 — catch when online. Probe 2026-07-09: unreachable; sheet claim "Done, v6/6.10.0" UNVERIFIED (NB: sheet said "v7" for 4734-106 but live showed v8 — sheet stale, always verify live). Prior: CCU IP from control sheet 2026-05-21 |
| 4734-111 | 11 | `10.179.39.1` | 🟢 **DONE — 15/15 visible APs at 6.11.2-0; 12/12 sw v8-011; OBN 8/8** | ❓ | **2026-07-09 AR: portal container NDE-2023Q4-10.1.7 → NDE-2025Q4-12.3.6** (`ndsu up` → 3b14f2e8 + reboot). Post-update fixes: vlan7 had regressed to `172.19.147.130` (box-id formula — ⚠️ t39 has NO `fis_id` in hieradata, vlan7 WILL regress on any future Puppet re-render until fis_id set) → restored `172.19.133.130` (ro-toggle + nmcli); Form-1 `{%- set train_id = 11 -%}` ADDED to 12/12 nv4-*.cfg (was never present on this train). `.dont` rename intact, timer inactive, 12/12 sw v8-011 up. ⚠️ FW vlan7 ARP INCOMPLETE (Q1 fail — Stadler FW not answering; vlan7 link clean; recheck next visit). Prior: **2026-05-22 sess 0951Z DONE:** Gate 4 obn_update_f approved 10:05Z. nohup serial AP fw push: 14/15 visible APs activated to 6.11.2-0 by 11:09Z; final .228 (Coach4 AP2m) pushed by orchestrator at 11:20Z (was staged-not-activated) → 6.11.2-0 ✅. **15/15 visible APs at 6.11.2-0**, 12/12 sw v8-011 fw 7.4.2, OBN 8/8. Coach3 AP1 not present in validate — verify if expected (4-car nv4 spec). Last touched: 2026-05-22 AR |
| 4734-112 | 12 | `10.179.37.1` | 🟡 PAUSED — **2026-07-26 AR: vlan7 REGRESSED AGAIN to `172.19.146.130` (box-id-37 encoding) → re-fixed to `172.19.134.2/17` via NDSU .dont chroot + reboot; post-flight A/B/C/D all pass (live=.134.2, FW `172.19.134.1` ARP REACHABLE Westermo, ICMP 100% loss = commissioned). ⚠️ WILL KEEP REGRESSING until the hieradata `fis_id` gap is closed (Puppet-side, DevOps) — see [[project_t37_t39_missing_fis_id_vlan7_regress]]. Also durably baked the outage-logger stack this session (run1, vign+netdrop active).** Prior: **2026-07-09 AR: portal container NDE-2023Q4-10.1.7 → NDE-2025Q4-12.3.6 ✅** (`ndsu up` staged; CCU dropped offline before manual reboot, then its own power-cycle activated the new snapshot). Post-activation: Form-1 survived (12/12 train_id=12), `.dont` intact, timer inactive, 12/12 sw up; **vlan7 had regressed to `172.19.146.130` (box-id formula — ⚠️ t37 has NO `fis_id` in hieradata, same durable gap as t39) → restored `172.19.134.2`, FW ARP `172.19.134.1` REACHABLE ✅ (Q1 pass).** Prior: 2026-06-09 sess orch9: CCU dropped ~11:50Z (sustained >5min, with 4736-120 simultaneously = consist/cellular outage, NOT CCU fault). PROGRESS THIS SESSION (all persisted): OBN 10/10 (run2/id339) ✅, train_id=12 directive re-promoted+rebooted ✅ (was lost in 1st promote from stale work-subvol; fixed), 12/12 sw nv4-*-v8-012 fw 7.4.2 ✅, vlan7 172.19.134.2/17 ✅. REMAINING: read rpcFwFlash on 8 m-APs (.219/.220/.225/.226/.227/.228/.230/.233) → push only rpcFwFlash=0; plain APs already 8/8 at 6.11.2-0. Resume: re-probe, no re-promote needed. Last touched 2026-06-09 AR | ✅ clear | 2026-06-08 sess 0900Z: 12/12 sw v8-012 fw 7.4.2 ✅, OBN 9/9 (run2/id333) ✅, vlan7 172.19.134.2/17 ✅. 8/8 plain APs at 6.11.2-0 ✅. 8 m-variant APs (coaches 3+4: .222/.224/.225/.226/.228/.230/.232/.233) staged-not-activated — TFTP confirmed, won_t partition-swap after 4 reboots; deferred run-wide, R&D ticket candidate. Last touched 2026-06-08 AR |
| 4734-113 | 13 | `10.179.46.1` | 🟡 **PAUSED — portal NDE-2025Q4-12.3.6 ✅ (2026-07-09 ndsu up+reboot, manual layer survived); chroot promote DONE (8/8 persisted incl bug9); next: Gate 3 obn_update_c (3 v7m sw), Gate 4 AP fw push** | ❓ | 2026-05-22 live probe: 12/12 sw on vlan100 ✅, vlan7 172.19.134.130/17 ✅, FW ARP INCOMPLETE (path not fully established). `obn validate`: 9/12 sw on v8 config ✓; 3 sw still v7m (G1 .180, G3 .183, E3/E2 .190/.192) ✗; all 16 APs at fw 6.10.0-0 ✗. Next: (1) re-apply TFTP helper + bug9 runtime; (2) re-run `obn update c all` to push remaining 3 sw; (3) AP fw push serial. |
...
| 4734-124 | 24 | `10.179.51.1` | ⚪ UNKNOWN | ❓ | initial visit |
...
| 4706-102 | 190 | `10.179.15.1` | 🟡 **PAUSED — SW config 18/18 fv6-v8-190 ✅ (2026-07-09 coupler fix); next: AP fw+config, Bug10 hand-port** | ❓ | **2026-07-09 AR — coupler switches FIXED: root cause = `#` comments in fv6 coupler templates (A1/A3/B1/B3), same bug as fv5 finding `RD_fv5_coupler_hash_comment_config_reject_2026-07-07` — switch config-test rejects any `#` line, boots back on old startup-config with exit-0-looking push. Manual push with `#`-strip (manual_sw_config_push.sh, B3→B1→A3→A1-last) → all 4 loaded first try → 18/18 `fv6-*-v8-190` verified via leases.** Portal container NDE-2025Q4-12.3.6 ✅, NDSU local b0d6e087_manual (behind remote 3b14f2e8 — fine, portal current). CCU had rebooted (TFTP helper wiped — pusher re-applies it; re-apply again before any future push). REMAINING: AP fw 6.10.0-0→6.11.2-0 + config→AP{N}-v1 (24/24 Nomad-form, no factory bypass); Bug10 hand-port on nd-obn 2.3.6; SW fw already at target 7.4.2 (7.4.8 NOT approved — do not push). Prior: **2026-07-06 AR — FV6 manual commission (dosto-commission-train is NV-only, can't drive FV6).** Done+persisted (subvol 690): `up`→fv6 templates 0.0.18; Form-1 `{%- set train_id = 190 -%}` in all 18 fv6-*.cfg → renders `fv6-*-v8-190` (verified); OBN patches **10/11** (Bug 10 PATTERN-NOT-FOUND on nd-obn **2.3.6** — needs hand-port; only guards `obn report` hang, inert on clean consist); vlan7 `172.19.223.2` ✓. **SW config push: 14/18 converged fv6-v8-190; 4 head/coupler switches A1(.188) A3(.189) B1(.184) B3(.186) still nv6-v7-143.** B3 re-pushed 2× incl. one full synchronous 5-min-hold run (exit 0, "applied/rebooting") but boots back on OLD startup-config every time → **real config-apply failure, NOT process-death** (ruled out). Fabric healthy: all 18 ping UP, vlan100 idle (4 pkt/s, 0 err), NO storm. Switch CLI returns blank (known VDS quirk — use leases). **NEXT: diagnose why B3 rejects config — verify TFTP RRQ from .186 actually reaches CCU during push (B3 IS in tftp_allowed ipset); check B3 startup vs running-config via SNMP.** Then finish A1/A3/B1 (A1 CCU-facing, last). **APs: 24/24 present, all Nomad-form `-v1-`, fw 6.10.0-0→target 6.11.2-0, config→AP{N}-v1; no factory-bypass needed.** Then SW fw 7.4.2→7.4.8, AP fw+config, Bug10 hand-port. Re-apply TFTP helper (runtime, wiped on reboot) before any push. See memory project_fv6_commission_manual_path. |
...
| 4705-103 | 231 | `10.179.41.1` | ⚪ UNKNOWN | ❓ | **2026-05-22 sess 0834: orchestrator pre-flight blocked — dosto-commission-train does not enumerate 4705 platform (only 4-car/6-car); switch hostname prefix fv5-* + ~14-15 sw observed suggest non-standard consist family. Skill update needed before commissioning.** sheet `Done`; live CCU confirmed at `.41.1` (switch hostnames `fv5-*-v3-231`); control sheet had `.42.1` typo (duplicate with 4705-101) **2026-07-09 AR: fv5 template 0.0.21 deployed to Puppet master (0.0.20 e1-11 fix + SW fw reverted to 7.4.2 — 7.4.8 not approved) (A2 e1-11 Bildschirm Werbung A4 was missing template-wide — findings/RD_fv5_A2_e1-11_bildschirm_missing_2026-07-09.md). **DONE 2026-07-09 PM (post-NDSU-up):** template 0.0.21 on CCU, Form-1 `train_id=231` re-added to all 15 fv5-*.cfg (ro-toggle; NDSU up had wiped it — re-apply after every template pkg upgrade), TFTP CT helper re-armed (runtime, wiped by reboot), `obn update c 10.179.41.180` pushed+rebooted A2, e1-11 verified live: vlan 3 / fis .228 / enabled. Port link DOWN — no powered device on the socket; ask Stadler/Fabian to power/connect the Werbung A4 display and confirm it leases 172.17.243.228. obn validate: 15/15 ✓ fw 7.4.2 ✓ configs -231 ✓ (7.4.8 exposure clean).** **2026-07-09 eve AR: vlan7 fixed PERSISTENTLY via chroot+safe_reboot — was 172.19.212.130 (broken formula, train_id=41 box-id), now 172.19.243.130/17 in nmconnection + live (snapshot run2). Form-1 train_id=231 baked into the same snapshot (chroot release copy did NOT have the ro-toggle fix — all 15 ADDED in-chroot; confirmed 15/15 post-reboot). TFTP CT helper re-armed post-reboot (still runtime-only). FW reach ✅: FW is at **172.19.243.129** (odd-Fzg device-1 addr, NOT .1) — Q1 ARP REACHABLE 00:90:e8:d7:18:15, Q2 ICMP 100% loss = **commissioned**, Q3 80/22 open. vlan7 fully verified end-to-end.** |
