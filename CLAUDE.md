...
bits 26-32 : Device          (7 bits, 1-127; CCU on vlan7 is always device 2; firewall is device 1)
```
...
Even Fzg → host `.2`. Odd Fzg → host `.130`. The Stadler firewall is **device 1 on the same `/17`, carrying the same odd-Fzg +128 bit**: `FW octet 4 = 128*(Fzg%2) + 1`. Even Fzg → FW `.1`, CCU `.2`; odd Fzg → FW `.129`, CCU `.130` (e.g. Fzg 133 vlan7 = `172.19.194.130/17`, Stadler FW = `172.19.194.129`). ⚠️ The FW is NOT always `.1` — probing `.1` on an odd-Fzg train false-classifies as path_broken. Field-verified 2026-07-09 on box1-t41 (4705-103, Fzg 231): FW at `172.19.243.129` ARP REACHABLE (`00:90:e8:d7:18:15`); `172.19.243.1` INCOMPLETE/no-route.
...
This phase answers **three separate questions** about the vlan7 connection to the Stadler FW peer — `172.19.X.1` on even-Fzg trains, `172.19.X.129` on odd-Fzg trains (see "vlan7 IP formula" above; the `172.19.X.1` in the commands below is the even-Fzg form — substitute `.129` for odd Fzg). Don't conflate them — each has its own test and its own success criterion. Conflating them silently produces wrong customer-report classifications (audit finding F9, 2026-05-11).
...
- **`dosto-l2-health` script `08_e2e_probe.sh` has a hardcoded default FW IP `172.19.196.1`.** For any train where the vlan7 FW is not at that address, the script produces a false `path_broken`. Always pass the FW IP explicitly: `bash 08_e2e_probe.sh <ccu-ip> <fw-ip>`. Correct FW IP per train = `172.19.<128+Fzg//2>.<128*(Fzg%2)+1>` (even Fzg → `.1`, odd Fzg → `.129`). Confirmed 2026-05-12 on Fzg 130 (FW = `172.19.193.1`); odd-Fzg `.129` case field-verified 2026-07-09 on Fzg 231 / box1-t41 (FW = `172.19.243.129`).
