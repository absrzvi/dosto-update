...
class DostoNeuReport(Report):
    def number_coaches(self):
        ...
        try:
            _dbg_sw=[d for d in self.device_instances.values() if d.type=="SW"]
            _dbg_un=[d for d in _dbg_sw if d.coach_number is None]
            self.logger.error("OBNFB-DIAG total=%d SW=%d unnumberedSW=%d cfgsample=%r",
                len(self.device_instances), len(_dbg_sw), len(_dbg_un),
                [(d.mac,getattr(d,"configuration_version",None)) for d in _dbg_un][:3])
            ...
            for _d in _fb_sw:
                ...
                if _d.mac not in _fb_reach:
                    continue
                _id = _fb_ident(getattr(_d, "configuration_version", None))
                ...
                self.logger.warning(
                    ...
                    _d.mac, getattr(_d, "configuration_version", "?"),
