...
class DostoNeuReport(Report):
    def number_coaches(self):
        ...
        # Robustness fallback. The hardcoded walk above numbers devices on
        # their EXPECTED wiring. A single missing inter-switch LLDP edge
        # (e.g. broken intra-coach cable) can leave a reachable switch (and
        # the APs behind it) unnumbered, dropping them from the inventory.
        # The link-down is already caught by the SNMP port alarm; OBN's job
        # is "if reachable, show it". Pass A recovers switches reachable via
        # ANY redundant LLDP switch path, identity from their own config
        # string. Pass B then numbers any AP hanging off a now-numbered
        # switch (e0-4 -> device = switch device; e1-2 -> device 4).
        try:
            ...
            # SW-SW undirected LLDP adjacency for reachability.
            _fb_adj = {}
            for _d in _fb_sw:
            ...
            # Pass A: recover unnumbered switches.
            for _d in _fb_sw:
                if _d.coach_number is not None or _d.mac not in _fb_reach:
                    continue
                ...
                self.logger.warning(
                    "OBNFALLBACK: switch %s (%s) -> coach %s device %s via redundant "
                    "LLDP path; OFF EXPECTED WIRING (check inter-switch cabling)",
                    _d.mac, getattr(_d, "config", "?"), _d.coach_number, _d.device_number,
                )

            # Pass B: number APs hanging off any now-numbered switch.
            for _d in _fb_sw:
                if _d.coach_number is None:
                    continue
                for _nb in _d.neighbours:
                    _ap = self.get_device(_nb)
                    if _ap is None or _ap.type != "AP" or _ap.coach_number is not None:
                        continue
                    _port = _nb.get("port", 0)
                    _ap.coach_number = _d.coach_number
                    if _port == 7:
                        _ap.device_number = _d.device_number
                    elif _port == 11 and _d.device_number == 3:
                        _ap.device_number = 4
                    if _ap.device_number is not None:
                        self.logger.warning(
                            "OBNFALLBACK: AP %s (%s) -> coach %s device %s via switch %s",
                            _ap.mac, getattr(_ap, "config", "?"),
                            _ap.coach_number, _ap.device_number, _d.mac,
                        )
                    else:
                        _ap.coach_number = None  # couldn't place device -> leave for normalise
